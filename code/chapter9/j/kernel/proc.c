...
// 打印进程状态
void print_process_status(struct proc* p, const char* queue_name) {
    disp_str("\n");
    disp_str(p->name);
    disp_str("[proc-ticks=");
    disp_int(p->ticks);
...
void dequeue(int queue[], int* queue_len) {
...
//     if (Q1_len != 0) { // Q1队列非空
//         int proc_id = Q1[0];
//         struct proc* p = &proc_table[proc_id];
//         p_proc_ready = p;

//         // 打印状态
//         print_process_status(p, "Q1");

//         p->queueticks--;

//         if (p->ticks <= 0) { // 进程完成
//             dequeue(Q1, &Q1_len);
//         } else if (p->queueticks <= 0) { // 时间片用完
//             dequeue(Q1, &Q1_len);
//             p->queueticks = Q2_TICK;
//             Q2[Q2_len++] = proc_id;
//         }
//     } else if (Q2_len != 0) { // Q2队列非空
//         int proc_id = Q2[0];
//         struct proc* p = &proc_table[proc_id];
//         p_proc_ready = p;

//         // 打印状态
//         print_process_status(p, "Q2");
...
//             Q3[Q3_len++] = proc_id;
//         }
//     } else if (Q3_len != 0) { // Q3队列非空
//         struct proc* p = 0;
//         int greatest_ticks = 0;

//         // 查找 Q3 中剩余 ticks 最大的进程
//         for (int i = 0; i < Q3_len; i++) {
//             struct proc* temp = &proc_table[Q3[i]];
//             if (temp->p_flags == 0 && temp->ticks > greatest_ticks) {
//                 greatest_ticks = temp->ticks;
//                 p = temp;
//             }
//         }
...
//             if (p->queueticks <= 0) {
//                 dequeue(Q3, &Q3_len);
//                 Q3[Q3_len++] = p - proc_table; // 使用进程索引重新入队
//                 p->queueticks = Q3_TICK;       // 重置时间片
//             }
//         }

//         // 如果所有进程 ticks 都耗尽，重置 ticks 为优先级
//         if (greatest_ticks == 0) {
//             for (int i = 0; i < Q3_len; i++) {
//                 struct proc* temp = &proc_table[Q3[i]];
//                 temp->ticks = temp->priority; // 重置 ticks
//             }
//         }
//     } else {
//         // 所有队列为空
// 		int a = 0;
//         disp_str("\nAll processes are completed. Exiting scheduling.\n");
//     }
// }
// PUBLIC void schedule()
// {
