...
int test() {
    // 初始化位图和页表
    my_memset(bitmap, 0, sizeof(bitmap));
    my_memset(page_table, 0, sizeof(page_table));

    // 请求分配5个页
    unsigned int pages_requested = 5;
    unsigned int page_table_address = alloc_pages(pages_requested);

    if (page_table_address != 0) {
        // 分配成功，进行其他操作

        // 释放已分配的页
        free_pages((unsigned int *)page_table_address, pages_requested);
    } else {
        // 分配失败，处理错误
    }
